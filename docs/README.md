# Documentation Index

**Find the right guide for your needs**

---

## Quick Start

| I want to... | Read this |
|--------------|-----------|
| Understand phext in 5 minutes | [Five-Minute Quickstart](./five-minute-quickstart.md) |
| See a narrative introduction | [Getting Started Story](./getting-started-story.md) |
| Sign up for SQ Cloud | [Signup Guide](./signup-guide.md) |

---

## Core Concepts

| Topic | Guide |
|-------|-------|
| Coordinate system (9D addressing) | [Coordinate Guide](./coordinate-guide.md) |
| Architecture overview | [Architecture](./architecture.md) |
| Delimiter characters | [Glyphmap](./glyphmap.md) |
| Terminology | [Glossary](./glossary.md) |

---

## Development

| I'm building... | Read this |
|-----------------|-----------|
| Any integration | [API Reference](./api-reference.md) |
| With Rust, Node, Python | [SDK Reference](./sdk-reference.md) |
| AI/LLM integration | [Integrations](./integrations.md) |
| Recipes and patterns | [Cookbook](./cookbook.md) |
| Production system | [Best Practices](./best-practices.md) |

---

## Migration & Comparison

| Coming from... | Read this |
|----------------|-----------|
| PostgreSQL, MongoDB, S3 | [Comparison Guide](./comparison-guide.md) |
| Other storage systems | [Migration Guide](./migration-guide.md) |

---

## Use Cases

| Domain | Guide |
|--------|-------|
| General use cases | [Use Cases](./use-cases.md) |
| Game development (Mytheon) | [Mytheon Arena Guide](./mytheon-arena-guide.md) |
| MUD/text adventure | [Phext MUD Design](./phext-mud-design.md) |
| Worldbuilding stories | [Portal Stories](./portal-stories.md) |

---

## Operations

| Task | Guide |
|------|-------|
| Run your own SQ instance | [Self-Hosted Guide](./self-hosted-guide.md) |
| Fix common issues | [Troubleshooting](./troubleshooting.md) |
| Security considerations | [Security](./security.md) |
| Quick command reference | [Quick Reference](./quick-reference.md) |

---

## Community

| Topic | Guide |
|-------|-------|
| Who we are | [About Us](./about-us.md) |
| Contributing, Discord, support | [Community Guide](./community-guide.md) |
| Where we're headed | [Roadmap](./roadmap.md) |
| Reading recommendations | [Reading Priorities](./reading-list-prioritization.md) |
| Launch communications | [Launch Comms](./launch-comms.md) |
| FAQ | [Extended FAQ](./faq-extended.md) |

---

## Reading Paths

### New to phext?
1. [Five-Minute Quickstart](./five-minute-quickstart.md)
2. [Coordinate Guide](./coordinate-guide.md)
3. [Signup Guide](./signup-guide.md)
4. [Best Practices](./best-practices.md)

### Building an integration?
1. [API Reference](./api-reference.md)
2. [SDK Reference](./sdk-reference.md)
3. [Cookbook](./cookbook.md)
4. [Troubleshooting](./troubleshooting.md)

### Evaluating SQ for your team?
1. [Getting Started Story](./getting-started-story.md)
2. [Comparison Guide](./comparison-guide.md)
3. [Use Cases](./use-cases.md)
4. [Security](./security.md)

### Deep dive?
1. [Architecture](./architecture.md)
2. [Coordinate Guide](./coordinate-guide.md)
3. [Glyphmap](./glyphmap.md)
4. [Reading Priorities](./reading-list-prioritization.md)

---

## Document Status

| Doc | Last Updated | Status |
|-----|--------------|--------|
| About Us | 2026-02-24 | ✅ New |
| API Reference | 2026-02-19 | ✅ Current |
| Architecture | 2026-02-20 | ✅ Current |
| Best Practices | 2026-02-25 | ✅ Updated |
| Community Guide | 2026-02-25 | ✅ Updated |
| Comparison Guide | 2026-02-22 | ✅ New |
| Cookbook | 2026-02-20 | ✅ Current |
| Coordinate Guide | 2026-02-19 | ✅ Current |
| Glyphmap | 2026-02-25 | ✅ Updated |
| Roadmap | 2026-02-24 | ✅ New |
| SDK Reference | 2026-02-22 | ✅ Current |
| Security | 2026-02-19 | ✅ Current |
| Self-Hosted Guide | 2026-02-23 | ✅ New |
| Troubleshooting | 2026-02-25 | ✅ Updated |

---

## Missing Something?

- Check [Extended FAQ](./faq-extended.md)
- Ask in [Discord #support](https://discord.com/invite/clawd)
- Email will@phext.io

---

*Last updated: 2026-02-25*
